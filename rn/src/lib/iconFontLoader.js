...
async function ensureIconFontLoaded({ name, assetModules = [], loadFont, fontFiles = [] }) {
  ...
  // Allow retrying font loads after transient startup/runtime failures.
  failedFonts.delete(name);
  ...
  const loadPromise = (async () => {
    try {
      ...
      const loadedByRuntime = await loadWithVectorIcons(loadFont, fontFiles);
...
export const MaterialCommunityIcons = getWrappedIconFamily("MaterialCommunityIcons");
