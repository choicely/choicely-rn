...
function WebRoot({
  components = {},
  initialComponent,
  forcedComponentName: forcedName,
  queryProps: providedQueryProps = {},
}) {
  ...
  if (forcedName) {
    const ForcedComponent = components[forcedName]?.registeredComponent
    return (
      <RootSafeArea>
        {ForcedComponent ? (
          <ForcedComponent {...providedQueryProps} />
        ) : (
          <MessageScreen text={`Component "${String(forcedName)}" not found`} />
        )}
      </RootSafeArea>
    )
  }
